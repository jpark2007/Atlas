...
- **No model refactor.** Do NOT add `Codable`/add `TaskStatus` raw values/restructure relationships in `Atlas/Models/Models.swift` for persistence. Persistence uses separate DTO structs. (Tasks MAY add *new optional fields* to a model only where the plan explicitly says so — Task 5 adds 3 optional fields to `CalendarEvent`.)
  - **AMENDMENT (2026-06-27, after Task 1):** changing `let id = UUID()` → `var id = UUID()` on `Space`, `Project`, `TaskItem`, `Note`, `Goal` IS permitted (and required) — it mirrors `CalendarEvent`'s existing `var id` and lets `AtlasDB.toDomain()` preserve the row's real id (set `.id` after constructing via the memberwise init). This is the ONLY sanctioned `id` change; do not add `Codable` or otherwise refactor these types. Done as part of the Task 1 fix wave.
